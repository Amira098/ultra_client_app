...
class ApiManager {
  Future<Result<T>> execute<T>(Future<T> Function() apiCall) async {
    try {
      ...
      return FailureResult<T>(
        exception: DataParsingException(
  ...
  Result<T> _handleDioException<T>(DioException e) {
    switch (e.type) {
      ...
      case DioExceptionType.receiveTimeout:
        return FailureResult<T>(
          exception: ApiTimeoutException(message: _getTimeoutMessage(e.type)),
        );
  ...
  Result<T> _handleBadResponse<T>(Response response) {
    ...
    switch (statusCode) {
      case 400:
        return FailureResult<T>(
          exception: BadRequestException(message: errorMessage, statusCode: statusCode),
        );
      case 401:
        return FailureResult<T>(
          exception: UnauthorizedException(message: errorMessage, statusCode: statusCode),
        );
      case 403:
        return FailureResult<T>(
          exception: ForbiddenException(message: errorMessage, statusCode: statusCode),
        );
      case 404:
        return FailureResult<T>(
          exception: NotFoundException(message: errorMessage, statusCode: statusCode),
        );
      case 422:
        if (response.data is Map<String, dynamic>) {
          final apiError = ApiError.fromJson(response.data);
          return FailureResult<T>(apiError: apiError);
        }
        return FailureResult<T>(
          exception: ValidationFailure(message: errorMessage),
        );
      case 500:
        return FailureResult<T>(
